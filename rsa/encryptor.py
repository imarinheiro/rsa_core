...
from rsa.report import set_context_key_encrypt_string, set_context_key_encrypt_number
...
def main():
    ...
    with open(args.path + 'report.json', 'w') as outfile:
        if args.type:
            json.dump(set_context_key_encrypt_number(args.text, args.key[0], args.key[1]), outfile, indent=4)
        else:
            json.dump(set_context_key_encrypt_string(args.text, args.key[0], args.key[1]), outfile, indent=4)
